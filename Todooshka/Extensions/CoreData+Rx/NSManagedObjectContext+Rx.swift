...
public extension Reactive where Base: NSManagedObjectContext {
  ...
  func delete<P: Persistable>(_ persistable: P) -> Observable<Void> {
    return Observable.create { observer in
      do {
        if let entity = try get(persistable) {
          self.base.delete(entity)
  ...
  func batchDelete<P: Persistable>(_ persistables: [P]) -> Observable<Void> {
    return Observable.create { observer in
      do {
  ...
  func entities<P: Persistable>(_ type: P.Type = P.self,
    ...
    return entities(fetchRequest: fetchRequest).map { $0.compactMap(P.init) }
  }
