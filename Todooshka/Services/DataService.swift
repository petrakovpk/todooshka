...
//  let firebaseKindsOfTask = BehaviorRelay<[Kind]?>(value: nil)
//  let firebaseTasks = BehaviorRelay<[Task]?>(value: nil)
//  let kindsOfTask: Driver<[Kind]>
...
//    .asDriver(onErrorJustReturn: [])
//
//    kindsOfTask = Driver.combineLatest(user, allKindsOfTask) { user, kindsOfTask -> [Kind] in
...
//          kindsOfTask.first { $0.uuid == task.kindOfTaskUID }?.style
//        }
//      }
