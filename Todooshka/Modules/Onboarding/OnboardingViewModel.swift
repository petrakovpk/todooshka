...
class OnboardingViewModel: Stepper {
  ...
  func transform(input: Input) -> Output {
    ...
    let onboardingSectionItem2 = OnboardingSectionItem(
      header: "Фабрика птиц",
      description: "Открывай новых птичек для каждого вида задач, пора пополнить свое гнездышко! Используй перышки и бриллианты, не заставляй Драго ждать.",
      image: UIImage(named: "Onboarding2") ?? UIImage()
    )
    let onboardingSectionItem3 = OnboardingSectionItem(
      header: "Календарь",
      description: "Будь в курсе дел на каждый день, не забывай проверять календарь",
