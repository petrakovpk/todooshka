...
class IdeaBoxTaskListViewModel: Stepper {
  ...
  func transform(input: Input) -> Output {
    ...
    let alertIsHidden = services.tasksService.taskRemovingIsRequired.map{ return $0 == nil }.asDriver(onErrorJustReturn: false)
    ...
    let alertCancelButtonClicked = input.alertCancelButtonClickTrigger.map{ self.services.tasksService.taskRemovingIsRequired.accept(nil) }
    
    let alertDeleteButtonClicked = input.alertDeleteButtonClickTrigger.map {
      guard let task = self.services.tasksService.taskRemovingIsRequired.value else { return }
      task.status = .deleted
