...
class TaskListCell: SwipeCollectionViewCell {
  static let reuseID: String = "TaskListCell"
  ...
  func configure(with item: TaskListSectionItem) {
    let mode = item.mode
    let task = item.task
    let kind = item.kind
    ...
    taskTypeImageView.image = kind?.icon?.image.template
    taskTypeImageView.tintColor = kind?.color
