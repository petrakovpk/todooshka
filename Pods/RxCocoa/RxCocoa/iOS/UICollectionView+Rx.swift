    ...
    */
    public func items<
        ...
        return { source in
            ...
            return source.subscribeProxyDataSource(ofObject: self.base, dataSource: dataSource, retainDataSource: true) { [weak taskListCollectionView = self.base] (_: RxCollectionViewDataSourceProxy, event) -> Void in
                guard let taskListCollectionView = taskListCollectionView else {
                    return
                }
                dataSource.collectionView(taskListCollectionView, observedEvent: event)
            }
        }
    }
}
